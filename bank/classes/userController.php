...
class user{
	...
	function authenticate_user(){
		...
		if(!$error){
			$result = $db->get_results( "select * from ".$apl->bank_table." where login_id = '".$login_id."' and password = '".md5($password)."' and status = 'Active' limit 1", OBJECT );

			if( count( $result ) > 0 ){
				$bank_data = $result[0];
				$this->current_user_id = $bank_data->bank_id;
