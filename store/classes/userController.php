...
class user{
	...
	public $current_user_login_id = null;
	...
	function __construct(){
		if( isset( $_SESSION['loggedIn'] ) ):
			if( isset( $_SESSION['current_user_id'] ) && $_SESSION['current_user_id'] != null ):
				...
				if( !empty( $user_data ) ):
					$this->current_user_id = $user_data->user_id;
					$this->current_user_login_id = $user_data->login_id;
				endif;
			endif;
		endif;
	}
	...
	function get_user_commission_details($user_id, $group_by = 'day'){
		global $apl;
		global $db;
		...
		if($user_details == false){
			...
			// $query = "select * from ".$apl->transaction_details_table." where transaction_name = 'commission_lottery_ticket' or transaction_name = 'zippcash_commission_wallet_recharge' and transaction_type = 'credit' and account_holder_id = '".$user_id."' ";
			if($group_by == "day"){
				$query = "SELECT YEAR(transaction_date) AS year, MONTH(transaction_date) AS month, DAY(transaction_date) AS day, SUM(transaction_amount) AS commission FROM $apl->transaction_details_table WHERE transaction_name = 'commission_lottery_ticket' OR transaction_name = 'store_commission_wallet_recharge' AND account_holder_id = '".$user_id."' AND transaction_date BETWEEN DATE(NOW()) - INTERVAL (DAY(NOW()) - 1) DAY - INTERVAL 11 MONTH AND NOW() GROUP BY DAY(transaction_date) ORDER BY YEAR(transaction_date) DESC, MONTH(transaction_date) DESC, DAY(transaction_date) DESC ";
			}
			if($group_by == "month"){
				$query = "SELECT YEAR(transaction_date) AS year, MONTH(transaction_date) AS month, DAY(transaction_date) AS day, SUM(transaction_amount) AS commission FROM $apl->transaction_details_table WHERE transaction_name = 'commission_lottery_ticket' OR transaction_name = 'store_commission_wallet_recharge' AND account_holder_id = '".$user_id."' AND transaction_date BETWEEN DATE(NOW()) - INTERVAL (DAY(NOW()) - 1) DAY - INTERVAL 11 MONTH AND NOW() GROUP BY MONTH(transaction_date) ORDER BY YEAR(transaction_date) DESC, MONTH(transaction_date) DESC, DAY(transaction_date) DESC ";
			}
			if($group_by == "year"){
				$query = "SELECT YEAR(transaction_date) AS year, MONTH(transaction_date) AS month, DAY(transaction_date) AS day, SUM(transaction_amount) AS commission FROM $apl->transaction_details_table WHERE transaction_name = 'commission_lottery_ticket' OR transaction_name = 'store_commission_wallet_recharge' AND account_holder_id = '".$user_id."' AND transaction_date BETWEEN DATE(NOW()) - INTERVAL (DAY(NOW()) - 1) DAY - INTERVAL 11 MONTH AND NOW() GROUP BY YEAR(transaction_date) ORDER BY YEAR(transaction_date) DESC, MONTH(transaction_date) DESC, DAY(transaction_date) DESC ";
			}
			$results = $db->get_results($query);
			$return_data->total_commission = 0;
			$return_data->commission_transaction = $results;
			if(count($results)){
				foreach ($results as $key => $value) {
					$return_data->total_commission += $value->commission;
				}
			}
			$return_data->currency = "Gourde";
		}
	...
	function get_current_user_account_details(){
		...
		if($current_user_details == false){
			...
			if($current_user_details->user_type != "store"){
				$error = true;
				$message = "You don't have authority to access this page. Your ID is " . $current_user_id;
			}else{
	...
	function get_user_list(){
		...
		$user_list = $db->get_results( "select * from ".$apl->user_table." where user_type = 'individual' and added_by_id = '".$this->current_user_id."'" );
		...
		if($current_user_details['success'] != false){
			if($current_user_details['data']->user_type == "store"){
				...
				if($user_data['user_type'] == 'individual'){
					...
					$error = true;
					$error_message[] = "You don't have permission to add a store.";
				}
				if(isset($_POST['email'])){
					if($_POST['email'] != null){
						if($this->is_email_registered($_POST['email'])){
				...
				if(!$error){
					...
					$user_data['verification_code'] = $apl->get_random_number( 6 );
					$user_data['password'] = md5($user_data['verification_code']);
					$user_data['added_by'] = "store";
					$user_data['added_by_id'] = $current_user_details['data']->user_id;
				...
				$error = true;
				$error_message[] = "You don't have access to add user";
			}
		}else{
			$error = true;
			$error_message[] = "You don't have access to add user";
	...
	function recharge_wallet($user_id, $credit_amount){
		...
		if($current_user_details['success'] == false){
			$error = true;
			$message = "You are not authorized to recharge user wallet";
		}else{
			if($current_user_details['data']->user_type != "store"){
				$error = true;
				$message = "Only stores have access to recharge user wallet.";
			}else{
		...
		$output['success'] = !$error;
		$output['data']  = $message;
		$output['userdetails']  = $current_user_details;
		return $output;
		die;
	}
	...
	function authenticate_user(){
		...
		if(!$error){
			$result = $db->get_results( "select * from ".$apl->user_table." where login_id = '".$login_id."' and password = '".md5($password)."' and user_type = 'store' limit 1", OBJECT );

			if( count( $result ) > 0 ){
				$store_data = $result[0];
				$this->current_user_id = $store_data->user_id;
